...
int get(List L){
	if(L == NULL) {
		fprintf(stderr, "err: List is undefined in get().\n");
		exit(EXIT_FAILURE);
	}
	if(L->index < 0 || L->cursor == NULL){
		fprintf(stderr, "err: Cursor is undefined in get().\n");
		exit(EXIT_FAILURE);
	}
...
void moveNext(List L){
	if(L == NULL || L->length < 1) {
			fprintf(stderr, "err: nList is undefined.\n");
			exit(EXIT_FAILURE);
	}
	if(L->index != -1) {
		if(L->cursor != L->back) {
...
void prepend(List L, int data){
	if(L == NULL) {
		fprintf(stderr, "err: pList is undefined.\n");
		exit(EXIT_FAILURE);
	}
...
void printList(FILE* out, List L){
	if(L == NULL || L->length <= 0) {
		fprintf(stderr, "err: pList is empty or undefined.\n");
		exit(EXIT_FAILURE);
	}
	Node itr = L->front;
...
List copyList(List L){
	if(L == NULL) {
		fprintf(stderr, "err: cList is empty or undefined.\n");
		exit(EXIT_FAILURE);
	}
